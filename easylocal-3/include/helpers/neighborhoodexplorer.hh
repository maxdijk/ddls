...
public:
     ...
     */
    void CopyDeltaCostComponents(const NeighborhoodExplorer<Input, Solution, Move, CostStructure> &ne)
    {
    ...
    /** Checks if a move in the neighborhood is legal.
     ...
     @param mv the move
     */
    virtual bool FeasibleMove(const Solution &st, const Move &mv) const
    {
        return true;
    }
     ...
     */
    virtual void RandomMove(const Solution &st, Move &mv) const = 0;
    ...
    /** Generates the first move in the neighborhood (a total ordering of the neighborhood is assumed). It is always used on cooperation with @ref NextMove to generate the whole neighborhood. It returns @c void because it is assumed that at least a move exists in the neighborhood.  It writes the first move in @c mv.
     ...
     @param mv the move
     */
    virtual void FirstMove(const Solution &st, Move &mv) const = 0;
    ...
    /** Generates the move that follows mv in the exploration of the neighborhood of the state st. It returns the generated move in the same variable mv.
     ...
     @param mv the move
     */
    virtual bool NextMove(const Solution &st, Move &mv) const = 0;
     ...
     */
    virtual void MakeMove(Solution &st, const Move &mv) const = 0;
     ...
     */
    virtual CostStructure DeltaCostFunctionComponents(const Solution &st, const Move &mv, const std::vector<double> &weights = std::vector<double>(0)) const;
     ...
     */
    virtual void AddDeltaCostComponent(DeltaCostComponent<Input, Solution, Move, CFtype> &dcc);
    ...
    virtual void AddCostComponent(CostComponent<Input, Solution, CFtype> &cc);
     ...
     */
    NeighborhoodExplorer(const Input &in, SolutionManager<Input, Solution, CostStructure> &sm, std::string name);
     ...
     */
    virtual EvaluatedMove<Move, CostStructure> SelectFirst(const Solution &st, size_t &explored, const MoveAcceptor &AcceptMove, const std::vector<double> &weights = std::vector<double>(0)) const;
    ...
    /**
     ...
     matches with the criterion expressed by the functional object bool f(const Move& mv, CostStructure cost)
     */
    virtual EvaluatedMove<Move, CostStructure> SelectBest(const Solution &st, size_t &explored, const MoveAcceptor &AcceptMove, const std::vector<double> &weights = std::vector<double>(0)) const;
    
    /**
     This method will select the first move in a random neighborhood exploration that
     matches with the criterion expressed by the functional object bool f(const Move& mv, CostStructure cost)
     */
    virtual EvaluatedMove<Move, CostStructure> RandomFirst(const Solution &st, size_t samples, size_t &explored, const MoveAcceptor &AcceptMove, const std::vector<double> &weights = std::vector<double>(0)) const;
    
    /**
     This method will select the best move in a random neighborhood exploration that
     matches with the criterion expressed by the functional object bool f(const Move& mv, CostStructure cost)
     */
    virtual EvaluatedMove<Move, CostStructure> RandomBest(const Solution &st, size_t samples, size_t &explored, const MoveAcceptor &AcceptMove, const std::vector<double> &weights = std::vector<double>(0)) const;
    
protected:
    const Input &in;                               /**< A reference to the input */
    SolutionManager<Input, Solution, CostStructure> &sm; /**< A reference to the attached state manager. */
    
    /** Lists of delta cost components (or adapters) */
    std::vector<DeltaCostComponent<Input, Solution, Move, CFtype> *> delta_hard_cost_components, delta_soft_cost_components;
    
    /** List of created adapters (to be automatically deleted in the destructor). */
    std::vector<std::shared_ptr<DeltaCostComponentAdapter<Input, Solution, Move, CostStructureType>>> dcc_adapters;
...
NeighborhoodExplorer<Input, Solution, Move, CostStructure>::NeighborhoodExplorer(const Input &i, SolutionManager<Input, Solution, CostStructure> &e_sm, std::string e_name)
 ...
 */
template <class Input, class Solution, class Move, class CostStructure>
CostStructure NeighborhoodExplorer<Input, Solution, Move, CostStructure>::DeltaCostFunctionComponents(const Solution &st, const Move &mv, const std::vector<double> &weights) const
{
...
template <class Input, class Solution, class Move, class CostStructure>
void NeighborhoodExplorer<Input, Solution, Move, CostStructure>::AddDeltaCostComponent(DeltaCostComponent<Input, Solution, Move, CFtype> &dcc)
{
    if (dcc.IsHard())
...
template <class Input, class Solution, class Move, class CostStructure>
void NeighborhoodExplorer<Input, Solution, Move, CostStructure>::AddCostComponent(CostComponent<Input, Solution, CFtype> &cc)
{
    
    dcc_adapters.push_back(std::make_shared<DeltaCostComponentAdapter<Input, Solution, Move, CostStructure>>(in, cc, *this));
    if (cc.IsHard())
    {
 ...
 */
template <class Input, class Solution, class Move, class CostStructure>
EvaluatedMove<Move, CostStructure> NeighborhoodExplorer<Input, Solution, Move, CostStructure>::SelectFirst(const Solution &st, size_t &explored, const MoveAcceptor &AcceptMove, const std::vector<double> &weights) const
{
    explored = 0;
 ...
 */
template <class Input, class Solution, class Move, class CostStructure>
EvaluatedMove<Move, CostStructure> NeighborhoodExplorer<Input, Solution, Move, CostStructure>::SelectBest(const Solution &st, size_t &explored, const MoveAcceptor &AcceptMove, const std::vector<double> &weights) const
{
 ...
 */
template <class Input, class Solution, class Move, class CostStructure>
EvaluatedMove<Move, CostStructure> NeighborhoodExplorer<Input, Solution, Move, CostStructure>::RandomFirst(const Solution &st, size_t samples, size_t &explored, const MoveAcceptor &AcceptMove, const std::vector<double> &weights) const
{
    EvaluatedMove<Move, CostStructure> mv;
    explored = 0;
 ...
 */
template <class Input, class Solution, class Move, class CostStructure>
EvaluatedMove<Move, CostStructure> NeighborhoodExplorer<Input, Solution, Move, CostStructure>::RandomBest(const Solution &st, size_t samples, size_t &explored, const MoveAcceptor &AcceptMove, const std::vector<double> &weights) const
{
    unsigned int number_of_bests = 0; // number of moves found with the same best value
    EvaluatedMove<Move, CostStructure> mv, best_move;
