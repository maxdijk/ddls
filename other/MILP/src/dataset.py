...
class Dataset:
    ...
    def __init__(self, csv_file, train_validation_test_split=[1.0, 0.0, 0.0], 
                 normalize_inputs=True, seed=5, verbose=False):
    ...
    def setup_train_test(self, train, test):
        self.X = self.df.iloc[:, :-1].values
        self.Y = self.df.iloc[:, -1].values

        self.classes = np.unique(self.Y)

        self.train_n = len(train)
        self.train_X = train.iloc[:, :-1].values
        self.train_Y = train.iloc[:, -1].values

        self.validation_n = 0

        self.test_n = len(test)
        self.test_X = test.iloc[:, :-1].values
        self.test_Y = test.iloc[:, -1].values


    # Used when we only want to use the entire dataset as training data
    def setup_train_only(self):
        self.X = self.df.iloc[:, :-1].values
        self.Y = self.df.iloc[:, -1].values

        self.classes = np.unique(self.Y)

        self.train_n = len(self.X)
        self.train_X = self.X
        self.train_Y = self.Y

        self.validation_n = 0

        # Does not work if no test samples so put a dummy sample
        self.test_n = 1
        self.test_X = [[1]]
        self.test_Y = [0]
