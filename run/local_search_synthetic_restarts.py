...
def random_restarts(params, X_train_full, filename_train_full, filename_test):
    alpha, width, depth = params

    structure = heuristic.get_structure(width, depth)
    heuristic.initialize_diagram(X_train_full, structure)
    heuristic.save_diagram(structure, True)
    ...
    for i in range(50):
        command = [
            ...
            '--main::initialization', "greedy",  # greedy for information gain initialization or random for random
            '--main::alpha', str(alpha),
            '--main::filename_train', filename_train_full,
            '--main::filename_test', filename_test,
            '--main::seed', str(random.randint(1, 99999)),
            '--main::random_move', '0.35',
            '--main::optimal_move', '0.6',
            '--main::edge_move', '0.05'
        ]
        ...
        print("RandomRestart" + target_line)
        objective_value = float(values[3])
        if objective_value < best_objective:
            best_output = target_line
            best_objective = objective_value

    print("Final" + best_output)
...
def run_HC(X_train_full, X_train, X_val, X_test, filename):
    ...
    random_restarts([best_alpha, best_width, best_depth], X_train_full, f"{filename}train_full.csv", f"{filename}test.csv")


csv_files = glob.glob("../instances_synthetic/*.csv")
filenames = [os.path.basename(f) for f in csv_files if "Train" in os.path.basename(f)][2:]
